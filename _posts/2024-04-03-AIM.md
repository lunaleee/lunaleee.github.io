...
patch features 집합 $P = \{p_i ∣ 1 ≤ i ≤ K\}$가 주어질 때, **<mark style='background-color: var(--hl-green)'><span style='color: var(--text-color)'>multi-head attention pooling</span></mark>**을 사용한 global descriptor $\hat{p}$ 는 다음과 같이 정의된다.
<br/>
...
각 attention head  $h = \{1, ..., H \}$에 대해 $W^h_k, W^h_v ∈ R^{d_h \times d}$ 는 각각 **key, value weight**을 나타낸다. $q_h$ 는 learnable한 **query vector**이다. 
